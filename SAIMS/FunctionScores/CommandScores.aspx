...
<body>
    ...
    <asp:Label ID="Label4" runat="server" style="color:Red;" 
            Text="使用说明（男宿部+女宿部）:输入量化先选择原因和宿舍楼 ，然后输入量化按照格式：“ 宿舍号+扣分加分数+床号”。例如：“101 1 4 5”一行分输入完成后按Enter输入下一行分数，不设上限但要确保正确。床号输入请如实输入。床号禁止输入-a的格式！！！！！！！！！！全选可不打床号 " 
        ...
        <asp:DropDownList ID="宿舍楼" runat="server" DataSourceID="SqlDataSource2" 
            ...
            onselectedindexchanged="宿舍楼_SelectedIndexChanged">
        </asp:DropDownList>
